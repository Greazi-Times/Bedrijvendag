    ...
    <main class="brand-hero min-h-screen">
        <section class="relative px-6 py-14 sm:py-20 lg:px-16">
            <div class="relative mx-auto grid max-w-7xl items-center gap-10 lg:grid-cols-[1.15fr_0.85fr]">
                <div class="max-w-3xl">
                    ...
                    <h1 class="mt-5 text-4xl font-semibold tracking-tight text-foreground sm:text-5xl">{{ t('companyAccess.title') }}</h1>
                    <p class="mt-5 max-w-2xl text-lg leading-relaxed text-muted-foreground">{{ t('companyAccess.intro') }}</p>
                    ...
                    <div class="mt-8 flex flex-wrap gap-3">
                        ...
                        >
                            {{ t('companyAccess.startEnrollment') }}
                ...
                <aside class="brand-card rounded-3xl p-6 sm:p-8" aria-labelledby="next-event-title">
                    <div class="flex items-start gap-4">
                        ...
                        <div>
                            <p id="next-event-title" class="text-sm font-semibold text-muted-foreground">{{ t('companyAccess.nextEvent') }}</p>
                            ...
                            <template v-else>
                                <h2 class="mt-1 text-xl font-semibold text-foreground">{{ t('companyAccess.dateComingSoon') }}</h2>
                                <p class="mt-2 text-sm text-muted-foreground">{{ t('companyAccess.stillInterested') }}</p>
                            </template>
                        </div>
                    </div>
        ...
        <section id="how-it-works" class="brand-section relative border-y border-border px-6 py-14 lg:px-16">
            <div class="mx-auto max-w-7xl">
                <div class="max-w-2xl">
                    <p class="brand-eyebrow">{{ t('companyAccess.processEyebrow') }}</p>
                    <h2 class="mt-4 text-3xl font-semibold tracking-tight">{{ t('companyAccess.processTitle') }}</h2>
                    <p class="mt-3 leading-relaxed text-muted-foreground">{{ t('companyAccess.processIntro') }}</p>
                </div>
                ...
                <ol class="mt-9 grid gap-5 md:grid-cols-3">
                    <li v-for="(icon, index) in [Send, MessagesSquare, FileText]" :key="index" class="brand-card rounded-2xl p-6">
                        ...
                        <h3 class="mt-5 text-lg font-semibold">{{ t(`companyAccess.step${index + 1}Title`) }}</h3>
                        <p class="mt-2 text-sm leading-relaxed text-muted-foreground">{{ t(`companyAccess.step${index + 1}Text`) }}</p>
                    </li>
                </ol>
                ...
                <div class="mt-8 grid gap-5 rounded-2xl bg-secondary/10 p-6 ring-1 ring-secondary/20 sm:grid-cols-2 sm:p-8">
                    <div class="flex gap-4">
                        ...
                        <div>
                            <h3 class="font-semibold">{{ t('companyAccess.whoYouMeet') }}</h3>
                            <p class="mt-2 text-sm leading-relaxed text-muted-foreground">{{ t('companyAccess.whoYouMeetText') }}</p>
                        </div>
                    </div>
                    <div class="flex gap-4">
                        <FileText class="mt-0.5 h-6 w-6 shrink-0 text-secondary" />
                        <div>
                            <h3 class="font-semibold">{{ t('companyAccess.whatToPrepare') }}</h3>
                            <p class="mt-2 text-sm leading-relaxed text-muted-foreground">{{ t('companyAccess.whatToPrepareText') }}</p>
                        </div>
                    </div>
                </div>
            </div>
        ...
        <section id="enroll" class="relative scroll-mt-24 px-6 py-14 sm:py-20 lg:px-16">
            <div class="mx-auto max-w-5xl">
                <div class="max-w-3xl">
                    <p class="brand-eyebrow">{{ t('companyAccess.formEyebrow') }}</p>
                    <h2 class="mt-4 text-3xl font-semibold tracking-tight text-foreground">{{ t('companyAccess.formTitle') }}</h2>
                    <p class="mt-3 text-base leading-relaxed text-muted-foreground">{{ t('companyAccess.formIntro') }}</p>
                </div>
                    ...
                    <div class="grid gap-6 lg:grid-cols-2">
                        <div>
                            ...
                            <input
                                ...
                                :placeholder="t('companyAccess.companyPlaceholder')"
                        ...
                        <div class="lg:col-span-2">
                            <label for="message" class="mb-2 block text-sm font-semibold text-foreground">{{ t('companyAccess.editionQuestion') }}</label>
                            <textarea
                                id="message"
                                v-model="form.message"
                                rows="4"
                                :placeholder="t('companyAccess.messagePlaceholder')"
